...
namespace DFMpr
{
	...
	void RGraphMonitor::blockBusPortsQueryUpdate(uint block_id_, uint port_id_)
	{
		_block_bus_ports_query[block_id_].push_back(port_id_);
	}

	auto RGraphMonitor::blockConnectionQueryUpdate(Cord source, Cord target) -> void
	{
		_block_connection_query[source].push_back(target);
	}
	...
	auto RGraphMonitor::queryBlockIndex(const Cord& block_pos_) const -> uint
	{
		...
		std::cout << block_pos_.x << block_pos_.y << std::endl;
		DEBUG_ASSERT(false);
		return UINT_MAX;
	}
	...
	auto RGraphMonitor::queryBlockConnection(Cord source_cord_) const -> vector<Cord>
	{
		if (_block_connection_query.find(source_cord_) == _block_connection_query.end())
			DEBUG_ASSERT(false)
			return _block_connection_query.find(source_cord_)->second;
	}
	// read
	...
	vector<uint> RGraphMonitor::queryBusPortsOfOneBlock(uint block_index_) const
	{
		auto itr = _block_bus_ports_query.find(block_index_);
		DEBUG_ASSERT(itr != _block_bus_ports_query.end());
		return itr->second;
	}
	...
	auto RGraphMonitor::queryPortIndex(const PortPos& port_pos_) const -> uint
	{
		...
		if (direction == PortDirection::in)
			candidates = queryInPortsOfOneBlock(block_index);
		else if(direction == PortDirection::out)
			candidates = queryOutPortsOfOneBlock(block_index);
		else
			candidates = queryBusPortsOfOneBlock(block_index);
