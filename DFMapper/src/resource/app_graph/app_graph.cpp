...
namespace DFMpr
{
	...
	auto AppGraph::init() -> void
	{
		...
		const vector<string> s1{ "pe","fg","ls","lv","lc","null","begin","bus","fifo" };
		...
		//add all DFGNODE to sim_nodes with their name and index(index in config)
		for (auto& [node_type, ptr_vec] : dfg)
		{
//			for (uint i = 0; i < ptr_vec.size(); i++)
//			{
				...
				else if (node_type == NodeType::bus)
					input_node_index = dynamic_cast<const Preprocess::DFGNode<Simulator::NodeType::bus>*>(ptr_vec)->index;
				else if (node_type == NodeType::fifo)
					input_node_index = dynamic_cast<const Preprocess::DFGNode<Simulator::NodeType::fifo>*>(ptr_vec)->index;
				else
					;
				if (node_type == NodeType::bus)
				{
					uint level = dynamic_cast<const Preprocess::DFGNode<Simulator::NodeType::bus>*>(ptr_vec)->level;
					if(level == 0)
						_nodes.emplace_back(NodeType::bus0, input_node_index);
					else
						_nodes.emplace_back(NodeType::bus1, input_node_index);
				}
				else
				{
					_nodes.emplace_back(node_type, input_node_index);
				}
				
				sim_nodes.emplace_back(node_type, input_node_index);//sim_nodes is in order of config,input_node_index is index in config
...
//			}
	...
	//		{
			...
			else if (input_node_type == NodeType::fifo)
				input_node_num = dynamic_cast<const Preprocess::DFGNode<Simulator::NodeType::fifo>*>(ptr_vec)->index;
			else
				;
			auto all_input = ptr_vec->getAllInput();
			uint input_node_index = Util::findIndex(sim_nodes, AppNode{ input_node_type, input_node_num });//input_node_index is in order of sim_nodes
			for (uint input_port_index = 0; input_port_index < all_input.size(); input_port_index++)
			{
				...
				_nodes[output_node_index].next_node.push_back(link);//////////应该将对应attribution->index的nodes push back进一个link,建立bridge的时候只是用来作为索引，因此link应该用列表索引
				_nodes[input_node_index].pre_node.push_back(link);
			}
		}
*/
