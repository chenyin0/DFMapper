...
namespace DFMpr
{
	...
	public:
		...
		auto getOutputAddr(OutputAddr type_) const -> string
		{
			...
			if (type_ == OutputAddr::logger_txt_addr)
				return _logger_txt_addr;
		...
		auto setOutputAddr(OutputAddr type_, string addr_) -> void
		{
			...
			else if (type_ == OutputAddr::logger_txt_addr)
				_logger_txt_addr = addr_;
			else
				DEBUG_ASSERT(false);
		}
	};
