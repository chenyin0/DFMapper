...
namespace DFMpr
{
	...
	vector<uint> RNodeBlock::getAllPreId() const
	{
		return pre_port_id;
	}
	...
	vector<uint> RNodePort::getAllPreId() const
	{
		vector<uint> result;
		result.insert(result.end(), pre_wire_id.begin(), pre_wire_id.end());
		result.insert(result.end(), pre_segment_id.begin(), pre_segment_id.end());
		result.insert(result.end(), pre_block_id.begin(), pre_block_id.end());
		result.insert(result.end(), pre_port_id.begin(), pre_port_id.end());
		return result;
	}
	...
	void RNodeWire::generateXmlInfoBrief(XMLDocument& doc_, XMLElement* root_) const
	{
		/*
		root_->SetAttribute("type", WireTypeConverter::toString(type).c_str());
		if(static_cast<string>(root_->Name()) == "pre")
			root_->SetAttribute("source_port_pos", static_cast<string>(source_port_pos).c_str());
		else if(static_cast<string>(root_->Name()) == "next")
			root_->SetAttribute("target_port_pos", static_cast<string>(target_port_pos).c_str());
	...
	vector<uint> RNodeWire::getAllPreId() const
	{
		vector<uint> result;
		result.push_back(pre_port_id);
		return result;
	}
	...
	vector<uint> RNodeSegment::getAllPreId() const
	{
		vector<uint> result;
		result.insert(result.end(), pre_port_id.begin(), pre_port_id.end());
		result.insert(result.end(), pre_segment_id.begin(), pre_segment_id.end());
