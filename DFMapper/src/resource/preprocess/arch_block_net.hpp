...
namespace Simulator::Preprocess
{
	...
	struct WireNet<WireNetConfigMode::cluster> : public WireNetInterface
	{
		...
		auto generate() const -> vector<WireGenerateBundle> override
		{
			...
			else if (block_type == BlockType::rt)
			{
				vector<Cord> temp_cords, rt_cords;
				for (uint i = 0; i < position.size(); ++i)
				{
					for (uint j = 0; j < position[i].size(); ++j)
					{
						if (position[i][j])
							temp_cords.push_back(Cord{ i,j });
					}
				}
				for (uint i = 1; i < temp_cords.size(); i = i + 2)
				{
					rt_cords.push_back(temp_cords[i]);
				}
				for (int i = temp_cords.size() - 2; i >= 0; i = i - 2)
				{
					rt_cords.push_back(temp_cords[i]);
				}
				uint num = 12;
				for (uint i = 0; i < rt_cords.size(); ++i)
				{
					for (uint j = num; j < 2 * num; ++j)
					{
						temp1.emplace_back(
							PortPos{ block_type, rt_cords[i], out_ports[j] },
							PortPos{ block_type, rt_cords[(i + 1) % 8], in_ports[j] }
						);
						temp2.emplace_back(
							PortPos{ block_type, rt_cords[(i + 1) % 8], bp_out_ports[j] },
							PortPos{ block_type, rt_cords[i], bp_in_ports[j] }
						);
					}

					for (uint j = 2 * num; j < 3 * num; ++j)
					{
						temp1.emplace_back(
							PortPos{ block_type, rt_cords[i], out_ports[j] },
							PortPos{ block_type, rt_cords[(i + 7) % 8], in_ports[j] }
						);
						temp2.emplace_back(
							PortPos{ block_type, rt_cords[(i + 7) % 8], bp_out_ports[j] },
							PortPos{ block_type, rt_cords[i], bp_in_ports[j] }
						);
					}
					for (uint j = 3 * num; j < 4 * num; ++j)
					{
						temp1.emplace_back(
							PortPos{ block_type, rt_cords[i], out_ports[j] },
							PortPos{ block_type, rt_cords[(i + 2) % 8], in_ports[j] }
						);
						temp2.emplace_back(
							PortPos{ block_type, rt_cords[(i + 2) % 8], bp_out_ports[j] },
							PortPos{ block_type, rt_cords[i], bp_in_ports[j] }
						);
					}
					for (uint j = 4 * num; j < 5 * num; ++j)
					{
						temp1.emplace_back(
							PortPos{ block_type, rt_cords[i], out_ports[j] },
							PortPos{ block_type, rt_cords[(i + 6) % 8], in_ports[j] }
						);
						temp2.emplace_back(
							PortPos{ block_type, rt_cords[(i + 6) % 8], bp_out_ports[j] },
							PortPos{ block_type, rt_cords[i], bp_in_ports[j] }
						);
					}
				}
