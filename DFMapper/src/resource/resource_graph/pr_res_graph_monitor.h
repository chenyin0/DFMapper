...
namespace DFMpr
{
	...
	class RGraphMonitor final
	{
		...
		/**
		* input a block node
		* return all this block's bus ports' indexes
		*/
		using BlockBusPortsQuery = pair<uint, vector<uint>>;
	...
	public:
		...
		auto blockBusPortsQueryUpdate(uint block_id_, uint port_id_) -> void;

		auto blockConnectionQueryUpdate(Cord source, Cord target) -> void;
		...
		auto queryBlockConnection(Cord source_cord_) const->vector<Cord>;
		...
		/**
		* find all bus port indexes of specific block
		*  */
		auto queryBusPortsOfOneBlock(uint block_index_) const->vector<uint>;
	...
	private:
		...
		unordered_map<uint, vector<uint>> _block_bus_ports_query;

		unordered_map<Cord, vector<Cord>> _block_connection_query;
	};
